...
def main():
    ...
    pprint.pprint(paths_list)

    lint.main(path = path, in_file= '%s/in.multi_bombard'%path, data_file = '%s/data.graphite_sheet'%path, reduce_size = True)
  
    #print("\n\nPATHSLIST: %s\n\n"%paths_list)

    #sim_dirs = sort(paths_list)

    #failed_sims = fail_checker(sim_dirs)

    #sim_dirs = [sim_dir for sim_dir in sim_dirs if sim_dir.success == True]
    #surface_finder_dir = [sim_dir for sim_dir in sim_dirs if sim_dir.surface_finder == True][0]

    #if len([surface_finder_dir]) == 1:

    #velocities = [sim_dir.velocity for sim_dir in sim_dirs]
    #velocities = list(set(velocities))

    #vel_dirs = [Vel_Dir(velocity, None) for velocity in velocities]

    #for vel_dir in vel_dirs:
    #    sorted_sim_dirs = []
    #    for sim_dir in sim_dirs:
    #        if vel_dir.velocity == sim_dir.velocity:
    #            sorted_sim_dirs.append(sim_dir)
                
    #    setattr(vel_dir, 'sim_dirs', sorted_sim_dirs)
    #    vel_dir.update()



    bulk_atoms_dict, graphite_sheets, atom_type, diamond_type = gmak.main(paths_list[0],
                                                                data_file_name=None, 
                                                                input_file_name="in.multi_bombard", 
                                                                new_data_file_name= None)

    results = tracker.main(all_xyz_file_path = "%s/all.xyz"%paths_list[0], no_of_sheets = graphite_sheets, atom_type = atom_type, 
                            diamond_type = diamond_type, energy = None)


    count = 1
    while True: #creating new directory for all velocities
        try:
    ...
    #for vel_dir in vel_dirs:    
     #   shutil.move(vel_dir.path, new_path)

    #plot.main(results_list, new_path)

    #os.remove("%s/file_paths.txt"%path)

    with open("%s/results.txt"%new_path, 'w') as fp:
        fp.write(results.present())

    #else:
        #print("\nERROR: Error in surface probe run. May need to be repeated.")
